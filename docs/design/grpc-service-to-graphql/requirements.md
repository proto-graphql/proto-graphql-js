...
- **公開 RPC**: `(graphql.service)` オプションでオプトインされたサービスに属し、`(graphql.rpc).ignore` されていない unary RPC
...
### R1. オプトインと対象選別
- R1.1: システムは `(graphql.service)` オプションが付与されたサービスの RPC のみを生成対象とすること(付与されないサービスは従来通り無視)
- R1.2: `(graphql.rpc).ignore = true` の RPC は生成対象から除外すること
- R1.3: streaming RPC(client/server/bidi)は生成対象外とし、警告を出力すること。明示的に `operation` が指定されている場合はエラーとすること
- R1.4: 本機能は `protobuf_lib=protobuf-es`(v2)でのみ有効とすること。他ランタイムで `(graphql.service)` を検出した場合はエラーとすること

### R2. operation 種別の決定
- R2.1: `(graphql.rpc).operation` が指定されていればそれに従うこと(QUERY / MUTATION)
- R2.2: 未指定の場合、`idempotency_level = NO_SIDE_EFFECTS` の RPC は Query、それ以外は Mutation として生成すること
...
### R10. proto 拡張オプション
- R10.1: 新オプション(`(graphql.service)`, `(graphql.rpc)`(`batch` を含む), `GraphqlFederationOptions` 等)は proto-graphql 本家リポジトリの `graphql/schema.proto` に experimental 明記で追加すること(field number 2056 慣例、Go binding 同時再生成、submodule 更新後 `pnpm gen:extensions`)
