...
|---|---|---|---|---|
| **entity** | `batch: {}` | `DataLoader<K, Entity \| null>` | `null` | BatchGet(1 キー → 高々 1 entity)。federation の resolveReference |
| **group** | `batch: { group: true }` | `DataLoader<K, Entity[]>` | `[]` | 1 キー → N entities(例: `user_id` → `Review[]`)。リレーション/extend フィールド |

## 2. proto オプション

`(graphql.rpc).batch` として宣言する(federation 非依存。proto-graphql 本家 `graphql/schema.proto` に experimental で追加)。
...
message GraphqlRpcBatchOptions {
  ...
  // entity モード: 省略時 (graphql.object_type).federation.key が単一フィールド fieldset ならフォールバック。
  // group モード: 必須(親キーは entity 自身の @key ではないため)
  string entity_key = 3;
...
service UserService {
  ...
  rpc BatchGetUsers(BatchGetUsersRequest) returns (BatchGetUsersResponse) {
    option (graphql.rpc).batch = {};  // key_field=ids, entity_field=users, entity_key=@key を推論
  };
}
...
| V5 | entity モードで `entity_key` 省略時: entity の `(graphql.object_type).federation.key` が**単一フィールドの fieldset 1 つ**ならそれを採用 | 複合キー・複数キーセット・@key なしはエラー(明示を要求) |
...
| V9 | **key_field 以外の request フィールドは loader パラメータになる**(§4.5)。required(`// Required.` behavior comment。判定は codegen-core の `isRequiredField` input セマンティクスを再利用)なフィールドが 1 つでもあれば、accessor の params 引数を**型レベルで必須**にする | エラーではなくシグネチャに反映 |
...
import { createRpcLoader } from "@proto-graphql/connect-runtime"; // 名称仮
...
// 非キーフィールドがある場合(§4.5): batchGetUsersLoader(ctx, { view: View.FULL }).load("user-1")
```

```ts
// review_service.pb.dataloader.ts(group モード)
export const batchListReviewsByUsersLoader = createRpcLoader({
  service: ReviewService,
  method: "batchListReviewsByUsers",
  group: true,
  maxBatchSize: 100, // batch.max_batch_size 指定時のみ出力
  call: (client, keys, opts) =>
    client.batchListReviewsByUsers(create(BatchListReviewsByUsersRequestSchema, { userIds: [...keys] }), opts),
...
export function createRpcLoader<S extends GenService, K, E, P>(config: {
  ...
  group?: false;
  maxBatchSize?: number;
}): (ctx: ProtoGraphqlConnectContext, params?: P) => DataLoader<K, E | null>;
// group オーバーロード: config.group: true → (ctx, params?) => DataLoader<K, E[]>
```

実装仕様:

1. **per-context キャッシュ(二段)**: loader accessor 毎に `WeakMap<object /*ctx*/, Map<string /*paramsKey*/, DataLoader>>` を持つ。同一 ctx + 同一 params の `load()` だけが同じ DataLoader(= 同じバッチ)に合流する。paramsKey の算出は §4.5
...
export const batchGetUsersLoader: (
  ...
  params?: BatchGetUsersLoaderParams, // required な非キーフィールドがあれば必須引数(params: ...)として生成
) => DataLoader<string, User | null>;
```

- required 判定は behavior comment(`// Required.`)ベース(V9)。**型レベルの強制のみ**で、runtime バリデーションはしない(不正はサーバが拒否する)
- 非キーフィールドが 1 つも無ければ従来どおり `(ctx) => DataLoader`(params 引数なし)
...
**注意(docs に明記)**: params をキーごとに変える使い方はバッチを分割し N+1 に近づく。これは仕様上正しい挙動(異なる入力は同一 RPC に同居できない)。
