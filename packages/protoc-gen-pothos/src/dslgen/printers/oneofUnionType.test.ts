...
function generateOneofUnionTypeCode(packageName: TestapisPackage, typeNameInProto: string, oneofFieldName: string, options: PothosPrinterOptions): string {
...
function generateSquashedOneofUnionTypeCode(packageName: TestapisPackage, typeNameInProto: string,  options: PothosPrinterOptions): string {
  const typeOptions: TypeOptions = {
    partialInputs: false,
    scalarMapping: options.protobuf === "ts-proto" ? defaultScalarMappingForTsProto : defaultScalarMapping,
    ignoreNonMessageOneofFields: false,
...
describe("createOneofUnionTypeCode", () => {
  describe("ts-proto", () => {
    ...
    test("generates code for a squashed oneof union", () => {
      const code = generateSquashedOneofUnionTypeCode("testapis.extensions", "PrefixedMessage.SquashedMessage",  options);
      expect(code).toMatchSnapshot();
    });

    test("generates code for imported oneof member", () => {
      const code = generateOneofUnionTypeCode("testapis.edgecases.import_oneof_member_from_other_file", "OneofParent", "oneof_field", options);
      expect(code).toMatchSnapshot();
    });
  });
